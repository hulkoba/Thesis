...
%
Für die nachfolgenden Aufrufe gibt es mehrere Varianten. Eine Anleitung zu dem Umgang mit Konflikten in der PouchDB Dokumentation empfiehlt für das Erstellen, Aktualisieren und Löschen eines Dokuments das Modul PouchDB Upsert zu verwenden. Das wird in Zeile eins zum PouchDB Objekt hinzugefügt.
...
Wie die Daten mit Redux Offline gespeichert synchronisiert werden ist am folgenden Listing erklärt. Das Beispiel erklärt das Hinzugügen eines Kontakts. Die restlichen Operationen, das Bearbeiten, Löschen oder Lesen eines Kontakts sind analog dazu. 
...
\end{center}
%
Mit dem Aufruf der Aktion ADD\_CONTACT, die in Zeile drei bis 15 erzeugt wird, wird der Vorgang einen Kontakt hinzuzufügen gestartet.
Die anzufragende URL ist im \tt{effect} Feld des \tt{meta.offline} Objekts in Zeile neun festgelegt. Die Anfrage geht an den Server, welcher die Daten in der \gls{JSON} Datei persisitiert.
...
Ist die Netzwerkanfrage erfolgreich, wird die Aktion \tt{commit} in Zeile zwölf gefeuert.
...
Hier könnte man die BenutzerInnen darüber informieren, dass etwas nicht geklappt hat. Für den Rahmen dieser Arbeit genügt es jedoch, wie es in den Zeilen zehn und elf steht, den vorherigen Status zurückzuliefern.\\\\
